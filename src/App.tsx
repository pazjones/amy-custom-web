...
import { ChevronLeft, ArrowRight, Instagram, Mail, Menu, X, Shirt, Brush, ShieldCheck, Send, ExternalLink } from 'lucide-react';
...
const ArtworkDetail: React.FC<{ artworks: Artwork[] }> = ({ artworks }) => {
  ...
  // Generamos el link de PayPal.me con el precio automático
    ...
    <div className="pt-40 pb-32 px-6 max-w-7xl mx-auto animate-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-28 items-start">
        ...
        <div className="lg:sticky lg:top-40">
          ...
          {/* BOTÓN PAYPAL.ME PERSONALIZADO */}
          <a 
            href={paypalMeLink} 
            target="_blank" 
            rel="noopener noreferrer"
            className="group relative flex items-center justify-center gap-4 bg-[#0070ba] text-white py-8 rounded-[30px] font-black uppercase tracking-[0.2em] text-lg shadow-2xl hover:bg-[#005ea6] transition-all transform hover:-translate-y-2"
          >
            Pagar con PayPal
            <ExternalLink size={22} className="group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
          </a>

          <div className="mt-12 flex flex-col gap-4 items-center text-[11px] font-black uppercase tracking-[0.2em] text-gray-400">
             <div className="flex items-center gap-2">
               <ShieldCheck size={18} className="text-green-500" /> Transacción segura vía PayPal
             </div>
             <p className="text-center opacity-60">Tras el pago, envíame un mensaje con tu comprobante para recibir tu archivo.</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <Router>
      <div className="min-h-screen bg-white">
          ...
          <Route path="/" element={
            <>
              <header className="pt-56 pb-24 px-6 flex flex-col items-center text-center">
                ...
                <p className="text-xl md:text-3xl font-bold text-gray-500 max-w-3xl leading-relaxed">Ilustración digital con alma.</p>
              </header>
              <Gallery artworks={artworks} />
              <section id="servicios" className="py-40 bg-black text-white px-6 text-center">
                <h2 className="text-6xl font-black uppercase mb-10">Comisiones</h2>
                <a href="mailto:amandajones.arts@gmail.com" className="inline-block bg-white text-black px-12 py-6 rounded-full font-black uppercase tracking-widest">Contactar</a>
              </section>
            </>
          } />
        ...
        <footer className="py-20 text-center border-t border-gray-50">
          <p className="text-xs font-black uppercase tracking-widest text-gray-400">© 2025 Amy Custom</p>
        </footer>
      </div>
    </Router>
  );
